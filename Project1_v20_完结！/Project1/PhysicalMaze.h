...
private:
    ...
    // 私有方法：用于存储和读取墙数据
    void saveWallsToFile(const std::vector<std::pair<int, int>>& walls, int size) const;
    std::vector<std::pair<int, int>> loadRandomWallsFromFile(int size) ;
    bool hasStoredMazes(int size) const;
