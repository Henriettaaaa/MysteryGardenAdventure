...
//没有迷宫，只走相对路径时，格子是自由的，只要在virtualmaze的范围就可以
void VirtualMaze::handleKeyPressA_free(sf::Keyboard::Key key) {
    // 保存当前位置作为上一个位置
    lastPlayerPositionA = playerPositionA;
    ...
    // 检查新位置是否合法
    if (canMoveTo_free(newPositionA.first, newPositionA.second)) {
        playerPositionA = newPositionA;
        
    }
}

bool VirtualMaze::canMoveTo_free(int newRow, int newCol) const {
    if (newRow < 0 || newRow >= virtualSize || newCol < 0 || newCol >= virtualSize) {
        return false;
    }
    return true;
